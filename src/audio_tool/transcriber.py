...
class DiarizationModel(str, Enum):
    ...
    DIARIZATION_COMMUNITY = 'pyannote/speaker-diarization-community-1'  # Faster inference
...
def extract_audio_fingerprint(audio_segment, sr):
    ...
    chroma = librosa.feature.chroma_stft(y=audio_segment, sr=sr, hop_length=512, n_fft=2048)
...
class Transcriber:
    ...
    def _load_models(self):
        ...
        self.logger.info(f'Loading models (whisper={self.whisper_backend.value}, diarization={self.diarization_model.value})...')
        ...
        self.speaker_embedding_model = PretrainedSpeakerEmbedding(
            ...
            device=self.device if self.device != 'mps' else 'cpu',  # MPS not always supported
            token=self.huggingface_token,
        )
        ...
        if self.whisper_backend == WhisperBackend.WHISPER_TIMESTAMPED:
            ...
            self.whisper_model = whisper_timestamped.load_model(self.model_name, device='cpu')
            self.logger.info(f'Loaded Whisper model (whisper-timestamped): {self.model_name}')
        else:
    ...
    def _transcribe_with_whisper_timestamped(self, audio: np.ndarray, segment_start: float) -> dict:
            ...
            for seg in result['segments']:
                if 'words' in seg:
                    for word_info in seg['words']:
                        ...
                        confidence = word_info.get('probability', word_info.get('confidence', 0.0))
                        words.append({
                            'word': word_text,
                            'start': word_info['start'] + segment_start,
                            'end': word_info['end'] + segment_start,
    ...
    def detect_speech_segments(
        ...
        for chunk_start in range(0, int(duration), max(1, chunk_size - overlap)):
            ...
            temp_dir = Path(tempfile.gettempdir()) / 'transcription_v2'
            temp_dir.mkdir(exist_ok=True)
            temp_chunk_path = temp_dir / f'transcribe_chunk_{chunk_start}_{chunk_end}.wav'
            ...
            try:
                ...
                if hasattr(chunk_diarization, 'speaker_diarization'):
                    ...
                    diarization_iter = (
                        (turn, speaker) for turn, _, speaker in chunk_diarization.itertracks(yield_label=True)
                    )
                else:
                    self.logger.warning(f'Unknown diarization output type: {type(chunk_diarization)}')
                    diarization_iter = []
        ...
        if chunk_segments:
            ...
            for segment in chunk_segments[1:]:
                ...
                should_merge = (
                    ...
                    and (gap < merge_gap_threshold or (merged_duration < min_segment_duration and gap < 2.0))
                )
            ...
            segments = [s for s in merged_segments if (s['end'] - s['start']) >= min_segment_duration]
        else:
            segments = []

        self.logger.info(f'Detected {len(segments)} speech segments in {time.time() - start_time:.2f}s')
        return segments

    def transcribe_segments(self, audio_path: Path, segments: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        use_mlx = self.whisper_backend in (WhisperBackend.MLX_LARGE_V3, WhisperBackend.MLX_TURBO)
        ...
        for i, segment in enumerate(segments):
            ...
            try:
                if use_mlx:
                    ...
                    try:
                        result = self._transcribe_with_mlx(str(temp_segment_path), segment['start'])
                    finally:
                        if temp_segment_path.exists():
                            temp_segment_path.unlink()
                else:
                    result = self._transcribe_with_whisper_timestamped(segment_audio, segment['start'])
    ...
    def filter_bad_segments(self, segments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def enhanced_audio_classification(self, segment_audio, sample_rate):
        ...
        spectral_rolloff = np.mean(librosa.feature.spectral_rolloff(y=segment_audio, sr=sample_rate))
        spectral_bandwidth = np.mean(librosa.feature.spectral_bandwidth(y=segment_audio, sr=sample_rate))
    ...
    def analyse_non_speech(self, audio_path: Path, transcribed_segments: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for segment in non_speech_segments:
            ...
            if segment['classification'] == 'music':
                try:
                    fingerprint = extract_audio_fingerprint(segment_audio, self.sample_rate)
                    enhanced_embedding = extract_enhanced_music_embedding(segment_audio, self.sample_rate)
                    ...
                    self.logger.info(f'Music detected: {segment["start"]:.1f}s - {segment["end"]:.1f}s')
                except Exception as e:
    ...
    def extract_speaker_embeddings(self, audio_path: Path, segments: list[dict[str, Any]]) -> dict[str, list[float]]:
            ...
            if segment['type'] == 'speech' and 'speaker' in segment:
                ...
                if speaker_id not in speaker_embeddings:
                    ...
                    if len(segment_audio) > self.sample_rate * 0.5:
                        audio_tensor = torch.tensor(segment_audio).unsqueeze(0).unsqueeze(0)
    ...
    def transcribe(self, audio_path: Path, language: str = None) -> dict[str, Any]:
        ...
        speaker_embeddings = self.extract_speaker_embeddings(audio_path, speech_segments)
        ...
        results = {
            ...
            'statistics': {
                'total_speech_segments': len([s for s in all_segments if s['type'] == 'speech']),
                'total_speakers': len(speaker_embeddings),
                'total_speech_duration': sum(s['end'] - s['start'] for s in all_segments if s['type'] == 'speech'),
            },
        }

        self.logger.info('Processing complete!')
        self.logger.info(f'  Speakers: {results["statistics"]["total_speakers"]}')
        self.logger.info(f'  Speech duration: {results["statistics"]["total_speech_duration"]:.1f}s')
