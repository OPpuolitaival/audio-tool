...
def get_audio_metadata(audio_path: str) -> tuple[float, dict[str, Any]]:
    ...
    try:
        ...
        metadata = {'format': data.get('format', {}), 'streams': data.get('streams', [])}
...
def calculate_phase_correlation(audio_path: str, metadata: dict[str, Any]) -> Optional[float]:
    """
...
def analyze_audio_quality(
    ...
    """
    # Validate parameters
    if not isinstance(silence_thresh_db, (int, float)) or not isinstance(min_silence_len_sec, (int, float)):
    ...
    try:
        ...
        if result.returncode != 0:
            raise ValueError(f'FFmpeg analysis failed for {audio_path} (exit code {result.returncode})')
        ...
        timeline_pattern = re.compile(r't:\s*([\d.]+)\s+.*?M:\s*(-?[\d.]+)\s+S:\s*(-?[\d.]+)\s+I:\s*(-?[\d.]+)')
...
def check_audio_quality(
    ...
    if result.loudness_range_lu is not None and result.loudness_range_lu > loudness_range_warning:
        checks.append(
            QualityCheck(
    ...
    if result.histogram_0db is not None and result.histogram_0db > clipping_samples_warning:
        checks.append(
            QualityCheck(
                check_type='clipping',
                severity='error',
    ...
    if result.phase_correlation is not None and result.phase_correlation < phase_correlation_warning:
        checks.append(
            QualityCheck(
                check_type='phase',
                severity='error',
...
def format_result_text(result: AudioQualityResult, checks: list[QualityCheck]) -> str:
    ...
    lines.append(f'  Longest middle silence: {result.longest_middle_silence_sec:.2f} sec')
